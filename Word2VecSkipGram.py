...
def forward_pass(tgtWordVec,w1,w2):
    h = np.dot(tgtWordVec.T,w1)   #hidden layer
...
def vec_sim(vec, top_n, unique_words,index_word):
    v_w1 = vec
    word_sim = {}
...
for i in range(settings['epochs']):
    ...
    for w_t, w_c in training_data: #target word vector, context word vector
        y_pred, hiddenLayer, ouputLayer = forward_pass(w_t,w1,w2)
        #sum up errors for all the context words
        EI = np.sum([np.subtract(y_pred, word) for word in w_c], axis=0)
        w1,w2 = backward_pass(EI, hiddenLayer, w_t,settings['learning_rate'],w1,w2)
        
        '''
        The first part is the negative of the sum for all the elements in the output
        layer (before softmax). 
        The second part takes the number of the context words
        and multiplies the log of sum for all elements (after exponential) in the 
        output layer. 
        np.where(word == 1)[0][0] returns the index of the value being 1.
        '''
        
        loss += -np.sum([ouputLayer[np.where(word == 1)[0][0]] for word in w_c]) + len(w_c) * np.log(np.sum(np.exp(ouputLayer)))
        print('Epoch:', i, "Loss:", loss)

vec = word_vec('my',word_index,w1)
vec_sim(vec, 4,unique_words,index_word)